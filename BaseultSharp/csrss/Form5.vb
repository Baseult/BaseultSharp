...
Public Class Form5
    ...
    Private Sub Form3_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Label4.Font = New Font(Form9.pfc.Families(0), 11, FontStyle.Bold)
    ...
    Private Sub B1_Click(sender As Object, e As EventArgs) Handles B1.Click
        ...
        Dim request =
                CType(WebRequest.Create("https://127.0.0.1:" & Form9.port & "/lol-champ-select/v1/team-boost/purchase"),
                      HttpWebRequest)
        request.PreAuthenticate = True
        request.ContentType = "application/json"
        request.Method = "POST"
        request.Headers.Add("Authorization", "Basic " & Form9.auth)

        Try
            Dim httpResponse = CType(request.GetResponse(), HttpWebResponse)

            Using streamReader = New StreamReader(httpResponse.GetResponseStream())
                Dim xd As String = streamReader.ReadToEnd().ToString()
                TextBox2.Text = TextBox2.Text + vbCrLf + "[" + DateTime.Now.ToString("hh:mm:ss") +
                                "] Aramboost: Success!"
            End Using
        Catch
            If Form9.LeagueVersion.Equals("11.4") Then
                TextBox2.Text = TextBox2.Text + vbCrLf + "[" + DateTime.Now.ToString("hh:mm:ss") +
                                "] ERROR: Aramboost failed!"
                TextBox2.Text = TextBox2.Text + vbCrLf + "[" + DateTime.Now.ToString("hh:mm:ss") +
                                "] ERROR: Your Leagueclient somehow updated itself to Version 11.4 and it only works on 11.3 Close your Leagueclient and restart Baseult# and try again!"
            End If
            TextBox2.Text = TextBox2.Text + vbCrLf + "[" + DateTime.Now.ToString("hh:mm:ss") +
                            "] ERROR: Aramboost failed!"
            TextBox2.Text = TextBox2.Text + vbCrLf + "[" + DateTime.Now.ToString("hh:mm:ss") +
                            "] ERROR: Make sure to Execute it while in Champ Select!"
